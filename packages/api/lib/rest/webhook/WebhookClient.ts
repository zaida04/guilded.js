...
export class WebhookClient {
  ...
  public constructor(
    ...
    if (typeof webhookConnection === "string") {
      const destructuredWebhookURL =
        /guilded.gg\/webhooks\/([^/]+)\/([^/]+)/.exec(webhookConnection);
  ...
  public async send(
    ...
    const resEmbeds = transformEmbedToAPIEmbed(
      (contentIsObject ? content.embeds : embeds) ?? []
    );
    ...
    return this.rest
      .post<RESTPostWebhookResult, FormData | RESTPostWebhookBody>("", body)
      .then((data) => {
