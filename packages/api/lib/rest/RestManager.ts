...
export class RestManager {
	...
	public async make<T extends JSONB, B = RequestBodyObject, Q = never>(
		...
		try {
			...
			throw new Error(`Error while making API call, ${(error as Error).message.toString()}`);
		}
		...
		if (!response.ok) {
			...
			const parsedResponse: string = rawResponse;
			...
			if (requestOptions.method !== "HEAD" && response.status !== 204) {
				...
				try {
					errorMessage = JSON.parse(rawResponse).message;
				} catch {
