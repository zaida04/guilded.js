...
export class RestManager {
	 ...
	 */
	readonly emitter =
		new EventEmitter() as TypedEmitter<RestManagerEvents>;
		...
		if (
			...
			if (
				...
				if (
					...
					(this
						.options
						?.maxRatelimitRetryLimit ??
						3)
				) {
					throw new Error(
						"MAX REQUEST RATELIMIT RETRY LIMIT REACHED.",
					);
				}
			...
			const rawResponse: string =
				await response
					...
					.catch(
						() =>
							"Could not read underlying response body buffer",
