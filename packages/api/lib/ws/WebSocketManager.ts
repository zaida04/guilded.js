...
export class WebSocketManager {
	 ...
	 */
	emitter =
		new EventEmitter() as TypedEmitter<WebsocketManagerEvents>;
	...
	get wsURL(): string {
		return (
			this
				.options
				.proxyURL ??
			`wss://www.guilded.gg/websocket/v${this.version}`
		);
	}
	...
	connect(): void {
		this._debug(
			`Connecting to Guilded WS Gateway at url ${this.wsURL}.`,
		);
		...
		this.socket.on(
			"ping",
			this.onSocketPing.bind(
				this,
			),
		);
		this.socket.on(
			"pong",
			this.onSocketPong.bind(
				this,
			),
		);
		this.socket.on(
		...
		this.socket.on(
			...
			(
				...
				this._debug(
					`Error received from WS. ${err.message}`,
				);
				this.emitter.emit(
					"exit",
					"Gateway connection  closed due to error.",
				);
				this._handleDisconnect(
					{
	...
	destroy(): void {
		this._debug(
			"Destroying websocket connection.",
		);
		if (
			!this
				.socket
			...
			throw new Error(
				"There is no active connection to destroy.",
			);
		}
	...
	private onSocketMessage(
		...
		this._debug(
			`Received event ${EVENT_NAME}. ${packet}}`,
		);
		...
		switch (
			...
			case WSOpCodes.WELCOME: {
				this._debug(
					"Received welcome packet. Setting up heartbeat.",
				);
				this.heartbeater =
					new Heartbeater(
						this,
						(
							EVENT_DATA.d as WSPayload<"_WelcomeMessage">
						)
							.heartbeatIntervalMs,
					);
				this.emitter.emit(
	...
	private onSocketPing(): void {
		this._debug(
			"Ping request from Guilded received. Responding with a pong.",
		);
		this.socket?.pong();
	}
