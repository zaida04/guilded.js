	...
	execute(message: Message): unknown {
		...
		if (this.client.botMention) {
			if (this.client.botMention === message.content)
				return this.client.messages.send(message.channelId, {
					content: this.parsePrefix(message.serverId),
					replyMessageIds: [message.id],
				});
			// IF THE MESSAGE STARTS WITH BOT MENTION, USE MENTION AS PREFIX
			if (message.content.startsWith(this.client.botMention)) prefix = this.client.botMention;
		}
