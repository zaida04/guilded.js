...
export class CommandsMonitor extends Monitor {
	...
	logCommand(
		...
		console.log(
			`${bgBlue(
				`[${this.client.getTime()}]`,
			)} ${command} by ${user} in ${guild} with Message ID: ${
				message.id
			}`,
		);
	}
			...
			if (
				...
				this.logCommand(
					...
					command.parentCommand
						? `${command.parentCommand}-${command.name}`
						: command.name,
				);
				return;
			}
			...
			if (
				...
				const question =
					await this.client.messages.send(
						message.channelId,
						{
							content: `You were missing the **${
								argument.name
							}** argument which is required in that command. Please provide the **${
								argument.type ===
								"subcommand"
									? command.subcommands
											?.map(
