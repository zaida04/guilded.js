...
export class BotClient extends Client {
	...
	async loadFile(result: any, dir: string, collection: Collection<string, unknown>): Promise<void> {
	...
	async executeTask(task: Task): Promise<void> {
		...
		try {
			await task.execute();
			console.log(`${bgBlue(`[${this.getTime()}]`)} [TASK: ${bgGreen(black(task.name))}] executed in ${this.humanizeMilliseconds(Date.now() - started)}.`);
		} catch (error) {
			console.log(error);
		}
	}
	...
	processMonitors(message: Message): void {
		for (const [id, monitor] of this.monitors) {
			...
			if (monitor.ignoreOthers && message.authorId !== this.user?.botId) continue;
