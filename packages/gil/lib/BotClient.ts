...
export class BotClient extends Client {
   ...
   */
  sourceFolderPath =
    this.options.sourceFolderPath ?? path.join(process.cwd(), "src/");
  ...
  async loadFile(
    ...
    const piece = file.default
      ? new file.default(this, name)
      : new file(this, name);
  ...
  async init(): Promise<void> {
    ...
    await Promise.allSettled(
      [
        ...
        try {
          for await (const result of walk(
            this.options.monitorDirPath ?? path.join(this.sourceFolderPath, dir)
          )) {
            await this.loadFile(result, dir, collection);
          }
        } catch (error) {
  ...
  async executeTask(task: Task): Promise<void> {
    ...
    console.log(
      `${bgBlue(`[${this.getTime()}]`)} [TASK: ${bgYellow(
        black(task.name)
      )}] started.`
    );
    ...
    try {
      await task.execute();
      console.log(
        `${bgBlue(`[${this.getTime()}]`)} [TASK: ${bgGreen(
          black(task.name)
        )}] executed in ${this.humanizeMilliseconds(Date.now() - started)}.`
      );
    } catch (error) {
      console.log(error);
    }
  ...
  processMonitors(message: Message): void {
    for (const [id, monitor] of this.monitors) {
      if (monitor.ignoreBots && message.createdByWebhookId) continue;
      if (monitor.ignoreOthers && message.authorId !== this.user?.botId)
        continue;
      if (
        monitor.ignoreEdits &&
        message.updatedAt &&
   ...
   */
  async cooldownReached(
    message: Message,
    command: Command,
    options: RespondToCooldownOption
  ): Promise<unknown> {
    return message.reply(
      `You must wait **${this.humanizeMilliseconds(
        options.cooldown.timestamp - options.now
      )}** before using the *${command.fullName}* command again.`
    );
  }
}
