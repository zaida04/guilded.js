...
export class RestManager {
  ...
  public async make<T extends JSONB, B = RequestBodyObject, Q = never>(
    ...
    retryCount = 0,
    {
      returnAsText = false,
      bodyIsJSON = true,
    }: { bodyIsJSON?: boolean; returnAsText?: boolean } = {}
    ...
    const queryAppendedURL = data.query
      ? `${data.path}?${stringify(data.query)}`
      : data.path;
    ...
    try {
      ...
      throw new Error(
        `Error while making API call, ${error.message.toString()}`
      );
    }
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        const retryAfterTime = Number(
          response.headers.get("Retry-After") ?? 35
        );
      ...
      const rawResponse: string = await response
        .text()
    ...
    return [
      ...
      returnAsText
        ? response.text()
        : (response.json().catch(() => ({})) as Promise<T>),
    ];
  }

  public async get<T extends JSONB, Q = RequestBodyObject>(
    path: string,
    query?: Q,
    authenticated = true
  ): Promise<T> {
  ...
  public async post<T extends JSONB, B = RequestBodyObject>(
    path: string,
    body?: B,
    authenticated = true
  ): Promise<T> {
  ...
  public async delete<T extends JSONB, B = RequestBodyObject>(
    path: string,
    body?: B,
    authenticated = true
  ): Promise<T> {
  ...
  public async patch<T extends JSONB, B = RequestBodyObject>(
    path: string,
    body: B,
    authenticated = true
  ): Promise<T> {
  ...
  public async put<T extends JSONB, B = RequestBodyObject>(
    path: string,
    body?: B,
    authenticated = true
  ): Promise<T> {
