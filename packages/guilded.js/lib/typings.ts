...
export type OptionBody<T extends (...args: any[]) => unknown> = Parameters<T>[0]["requestBody"];
// biome-ignore lint/suspicious/noExplicitAny: Needed
export type OptionQuery<T extends (...args: any[]) => unknown> = Parameters<T>[0];
...
export type MessageContent =
	| Embed
	| OptionBody<ChatService["channelMessageCreate"]>
	| string
	| (Omit<OptionBody<ChatService["channelMessageCreate"]>, "embeds"> & {
			embeds?: Embed[];
	  });
...
export type ClientEvents = {
	...
	unknownGatewayEvent(data: unknown): unknown;
};
