...
export class Message extends Base<ChatMessagePayload> {
  ...
  readonly server: Promise<Server>;
  ...
  constructor(client: Client, data: ChatMessagePayload) {
    super(client, data);
    this.server = client.servers.fetch(data.serverId || "");
  ...
  async channel(): Promise<Channel | null> {
    return (await this.client.channels.fetch(this.channelId)) ?? null;
  }
  ...
  async url(): Promise<string> {
    ...
    const server: Server = await this.server;
