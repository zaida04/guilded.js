...
export class CalendarEvent extends Base<CalendarEventPayload, number> {
	...
	constructor(
		...
		this.isAllDay =
			data.isAllDay ??
			null;
		this.autofillWaitlist =
			data.autofillWaitlist ??
			null;
...
export class CalendarEventRsvp extends Base<CalendarEventRsvpPayload, string> {
	...
	constructor(
		client: Client,
		data: CalendarEventRsvpPayload,
	) {
		super(
			client,
			{
				...data,
				id: `${data.calendarEventId.toString()}-${
					data.userId
				}`,
			},
		);
	...
	_update(
		data: Partial<CalendarEventRsvpPayload>,
	): this {
		if (
			"updatedAt" in
				data &&
