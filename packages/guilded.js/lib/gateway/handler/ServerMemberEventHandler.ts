...
export class ServerMemberEventHandler extends GatewayEventHandler {
	serverMemberUpdated(data: WSPacket<"ServerMemberUpdated">): boolean {
		...
		member?._update({
			nickname: data.d.userInfo.nickname ?? undefined,
		});
	...
	serverMemberBanned(data: WSPacket<"ServerMemberBanned">): boolean {
		...
		if (this.client.options.cache?.cacheMemberBans !== false) this.client.bans.cache.set(buildMemberKey(newMemberBan.serverId, newMemberBan.target.id), newMemberBan);
	...
	serverMemberSocialLinkCreated(data: WSPacket<"ServerMemberSocialLinkCreated">): boolean {
		...
		const existingMember = this.client.members.cache.get(buildMemberKey(serverId, socialLink.userId));
		if (this.client.members.shouldCacheSocialLinks) existingMember?.socialLinks.set(socialLink.type, socialLink);

		return this.client.emit(constants.clientEvents.MEMBER_SOCIAL_LINK_CREATED, serverId, socialLink);
	}
	...
	serverMemberSocialLinkUpdated(data: WSPacket<"ServerMemberSocialLinkUpdated">): boolean {
		...
		return this.client.emit(constants.clientEvents.MEMBER_SOCIAL_LINK_UPDATED, serverId, socialLink);
	}
	...
	serverMemberSocialLinkDeleted(data: WSPacket<"ServerMemberSocialLinkDeleted">): boolean {
		...
		const existingMember = this.client.members.cache.get(buildMemberKey(serverId, socialLink.userId));
		if (this.client.members.shouldCacheSocialLinks) existingMember?.socialLinks.delete(socialLink.type);

		return this.client.emit(constants.clientEvents.MEMBER_SOCIAL_LINK_DELETED, serverId, socialLink);
	}
}
