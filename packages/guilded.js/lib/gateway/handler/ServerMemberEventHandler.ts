...
export class ServerMemberEventHandler extends GatewayEventHandler {
	serverMemberUpdated(
		data: WSPacket<"ServerMemberUpdated">,
	): boolean {
		const {
			d: {
				userInfo: {
	...
	serverMemberJoined(
		data: WSPacket<"ServerMemberJoined">,
	): boolean {
		const newMember =
			new Member(
				this
	...
	serverMemberRemoved(
		data: WSPacket<"ServerMemberRemoved">,
	): boolean {
		const memberKey =
			buildMemberKey(
				data
		...
		if (
			...
			this.client.members.cache.delete(
				memberKey,
			);
	...
	serverMemberBanned(
		data: WSPacket<"ServerMemberBanned">,
	): boolean {
		const newMemberBan =
			new MemberBan(
				this
	...
	serverMemberUnbanned(
		data: WSPacket<"ServerMemberUnbanned">,
	): boolean {
		const {
			d: {
				serverId,
	...
	serverMemberSocialLinkDeleted(
		...
		if (
			...
			existingMember?.socialLinks.delete(
				socialLink.type,
			);
