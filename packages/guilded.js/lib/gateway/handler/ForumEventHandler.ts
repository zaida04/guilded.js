...
import { ForumTopic, ForumTopicComment } from "../../structures/Forum";
import { GatewayEventHandler } from "./GatewayEventHandler";
import { buildCommentKey } from "../../util";
...
export class ForumEventHandler extends GatewayEventHandler {
  ...
  forumTopicCommentCreated(data: WSPacket<"ForumTopicCommentCreated">) {
    this.client.emit(
      "forumTopicCommentCreated",
      new ForumTopicComment(this.client, data.d.forumTopicComment)
    );
  }
  forumTopicCommentUpdated(data: WSPacket<"ForumTopicCommentUpdated">) {
    this.client.emit(
      "forumTopicCommentUpdated",
      new ForumTopicComment(this.client, data.d.forumTopicComment)
    );
  }
  forumTopicCommentDeleted(data: WSPacket<"ForumTopicCommentDeleted">) {
    this.client.emit(
