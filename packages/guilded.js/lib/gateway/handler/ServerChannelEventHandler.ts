...
export class ServerChannelEventHandler extends GatewayEventHandler {
	serverChannelCreated(
		data: WSPacket<"ServerChannelCreated">,
	): boolean {
	...
	serverChannelUpdated(
		data: WSPacket<"ServerChannelUpdated">,
	): boolean {
		const existingChannel =
			this.client.channels.cache.get(
				data
	...
	serverChannelDeleted(
		data: WSPacket<"ServerChannelDeleted">,
	): boolean {
		const existingChannel =
			this.client.channels.cache.get(
				data
		...
		if (
			this
				...
				.options
				.cache
				?.removeChannelOnDelete
		)
			this.client.channels.cache.delete(
				deletedChannel.id,
			);
		return this.client.emit(
			constants
