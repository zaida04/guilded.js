...
export class ServerChannelEventHandler extends GatewayEventHandler {
    ...
    serverChannelUpdated(data: WSPacket<"ServerChannelUpdated">): boolean {
        ...
        const updatedChannel = existingChannel?._update(data.d.channel) ?? new (transformTypeToChannel(data.d.channel.type))(this.client, data.d.channel);
    ...
    serverChannelDeleted(data: WSPacket<"ServerChannelDeleted">): boolean {
        const existingChannel = this.client.channels.cache.get(data.d.channel.id);
        const deletedChannel = existingChannel?._update({ ...data.d.channel, deleted: true }) ?? new (transformTypeToChannel(data.d.channel.type))(this.client, data.d.channel);
