...
export class MessageEventHandler extends GatewayEventHandler {
	async messageCreated(
		data: WSPacket<"ChatMessageCreated">,
	): Promise<boolean> {
	...
	messageUpdated(
		data: WSPacket<"ChatMessageUpdated">,
	): boolean {
		...
		const frozenOldMessage =
			Object.freeze(
				getCachedMessage._clone(),
			);
		getCachedMessage._update(
			data
				.d
				.message,
		);
	...
	messageDeleted(
		data: WSPacket<"ChatMessageDeleted">,
	): boolean {
