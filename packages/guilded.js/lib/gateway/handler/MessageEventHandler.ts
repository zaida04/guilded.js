...
export class MessageEventHandler extends GatewayEventHandler {
  async messageCreated(data: WSChatMessageCreatedPayload): Promise<boolean> {
    ...
    if (existingMessage)
      return this.client.emit(
        constants.clientEvents.MESSAGE_CREATED,
        existingMessage
      );
    if (
      (this.client.options.cache?.fetchMessageAuthorOnCreate ?? true) &&
      data.d.serverId &&
      data.d.message.createdBy &&
      data.d.message.createdBy !== "Ann6LewA"
    )
      await this.client.members
        .fetch(data.d.serverId, data.d.message.createdBy)
        .catch(() => null);
