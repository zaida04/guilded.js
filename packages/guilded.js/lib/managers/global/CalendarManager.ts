...
export class GlobalCalendarManager extends CacheableStructManager<number, CalendarEvent> {
	...
	async create(
		channelId: string,
		options: OptionBody<
			CalendarEventsService["calendarEventCreate"]
		>,
	): Promise<CalendarEvent> {
		const data =
			await this.client.rest.router.calendarEvents.calendarEventCreate(
				{
					channelId,
					requestBody: options,
				},
	...
	async fetch(
		...
		if (
			...
			const existingCalendar =
				this.client.calendars.cache.get(
					calendarEventId,
				);
			if (
				existingCalendar
		...
		const data =
			await this.client.rest.router.calendarEvents.calendarEventRead(
				{
					channelId,
					calendarEventId,
				},
		...
		if (
			...
			this.client.calendars.cache.set(
				newCalendar.id,
				newCalendar,
			);
		return newCalendar;
	...
	async fetchMany(
		...
		options: Omit<
			OptionQuery<
				CalendarEventsService["calendarEventReadMany"]
			>,
			"channelId"
		>,
	): Promise<
	...
	> {
		const data =
			await this.client.rest.router.calendarEvents.calendarEventReadMany(
				{
					channelId,
					...options,
				},
	...
	async update(
		...
		calendarEventId: number,
		options: OptionBody<
			CalendarEventsService["calendarEventUpdate"]
		>,
	): Promise<CalendarEvent> {
		const data =
			await this.client.rest.router.calendarEvents.calendarEventUpdate(
				{
					channelId,
					calendarEventId,
					requestBody: options,
		...
		if (
			...
			return existingCalendar._update(
				data.calendarEvent,
			);
		const newCalendar =
			new CalendarEvent(
	...
	async fetchRsvp(
		...
		const data =
			await this.client.rest.router.calendarEvents.calendarEventRsvpRead(
				{
					channelId,
					calendarEventId,
					userId,
	...
	> {
		const data =
			await this.client.rest.router.calendarEvents.calendarEventRsvpReadMany(
				{
					channelId,
					calendarEventId,
				},
	...
	async updateRsvp(
		...
		userId: string,
		options: OptionBody<
			CalendarEventsService["calendarEventRsvpUpdate"]
		>,
	): Promise<CalendarEventRsvp> {
		const data =
			await this.client.rest.router.calendarEvents.calendarEventRsvpUpdate(
				{
					channelId,
					calendarEventId,
					userId,
	 ...
	 */
	async updateManyRsvp(
		channelId: string,
		calendarEventId: number,
		options: OptionBody<
			CalendarEventsService["calendarEventRsvpUpdateMany"]
		>,
	): Promise<void> {
