...
export class GlobalCalendarManager extends CacheableStructManager<number, CalendarEvent> {
    ...
    create(channelId: string, options: OptionBody<CalendarEventsService["calendarEventCreate"]>): Promise<CalendarEvent> {
        return this.client.rest.router.calendarEvents.calendarEventCreate({ channelId, requestBody: options }).then((data) => {
    ...
    fetch(channelId: string, calendarEventId: number, force?: boolean): Promise<CalendarEvent> {
        ...
        return this.client.rest.router.calendarEvents.calendarEventRead({ channelId, calendarEventId }).then((data) => {
    ...
    fetchMany(channelId: string, options: Omit<OptionQuery<CalendarEventsService["calendarEventReadMany"]>, "channelId">): Promise<Collection<number, CalendarEvent>> {
        return this.client.rest.router.calendarEvents.calendarEventReadMany({ channelId, ...options }).then((data) => {
    ...
    update(channelId: string, calendarEventId: number, options: OptionBody<CalendarEventsService["calendarEventUpdate"]>): Promise<CalendarEvent> {
        return this.client.rest.router.calendarEvents.calendarEventUpdate({ channelId, calendarEventId, requestBody: options }).then((data) => {
    ...
    delete(channelId: string, calendarEventId: number): Promise<CalendarEvent | void> {
        return this.client.rest.router.calendarEvents.calendarEventDelete({ channelId, calendarEventId }).then((data) => {
    ...
    fetchRsvp(channelId: string, calendarEventId: number, userId: string, force?: boolean): Promise<CalendarEventRsvp> {
        ...
        return this.client.rest.router.calendarEvents.calendarEventRsvpRead({ channelId, calendarEventId, userId }).then((data) => {
    ...
    fetchManyRsvps(channelId: string, calendarEventId: number): Promise<Collection<string, CalendarEventRsvp>> {
        return this.client.rest.router.calendarEvents.calendarEventRsvpReadMany({ channelId, calendarEventId }).then((data) => {
    ...
    deleteRsvp(channelId: string, calendarEventId: number, userId: string): Promise<CalendarEventRsvp | void> {
        return this.client.rest.router.calendarEvents.calendarEventRsvpDelete({ channelId, calendarEventId, userId }).then(() => {
            if (this.shouldCacheCalendar && this.shouldCacheCalendarRsvps) {
                const cachedCalendar = this.cache.get(calendarEventId);
