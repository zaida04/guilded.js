...
export class GlobalRoleManager extends CacheableStructManager<number, Role> {
    ...
    create(serverId: string, options: OptionBody<RolesService["roleCreate"]>): Promise<Role> {
        return this.client.rest.router.roles.roleCreate({ serverId, requestBody: options }).then((data) => {
    ...
    fetch(serverId: string, roleId: number, force?: boolean): Promise<Role> {
        ...
        return this.client.rest.router.roles.roleRead({ serverId, roleId }).then((data) => {
    ...
    fetchMany(serverId: string): Promise<Collection<number, Role>> {
        return this.client.rest.router.roles.roleReadMany({ serverId }).then((data) => {
    ...
    update(serverId: string, roleId: number, options: OptionBody<RolesService["roleUpdate"]>): Promise<Role> {
        return this.client.rest.router.roles.roleUpdate({ serverId, roleId, requestBody: options }).then((data) => {
    ...
    delete(serverId: string, roleId: number): Promise<Role | void> {
        return this.client.rest.router.roles.roleDelete({ serverId, roleId }).then(() => {
    ...
    updatePermissions(serverId: string, roleId: number, options: OptionBody<RolesService["rolePermissionUpdate"]>): Promise<void> {
        return this.client.rest.router.roles.rolePermissionUpdate({ serverId, roleId, requestBody: options }).then(() => void 0);
    }
    ...
    addRoleToMember(serverId: string, userId: string, roleId: number): Promise<void> {
        return this.client.rest.router.roleMembership.roleMembershipCreate({ serverId, userId, roleId }).then(() => void 0);
    }
    ...
    removeRoleFromMember(serverId: string, userId: string, roleId: number): Promise<void> {
        return this.client.rest.router.roleMembership.roleMembershipDelete({ serverId, userId, roleId }).then(() => void 0);
    }
}
