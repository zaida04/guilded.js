import type { CollectorOptions, MessageReaction } from "../../structures";
...
export class GlobalReactionManager extends CacheableStructManager<string, MessageReaction> {
    ...
    create(channelId: string, messageId: string, emoteId: number): Promise<void> {
        return this.client.rest.router.reactions.channelMessageReactionCreate({ channelId, messageId, emoteId }).then(() => void 0);
    }
    ...
    delete(channelId: string, messageId: string, emoteId: number, userId?: string): Promise<void> {
        return this.client.rest.router.reactions.channelMessageReactionDelete({ channelId, messageId, emoteId, userId }).then(() => void 0);
    }
     ...
     */
    awaitReactions(messageId: string, options: CollectorOptions<MessageReaction>) {
