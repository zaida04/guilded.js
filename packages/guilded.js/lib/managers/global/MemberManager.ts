...
> {
  ...
  async fetchMany(
    serverId: string
  ): Promise<Collection<string, PartialMember>> {
  ...
  async kick(serverId: string, userId: string): Promise<Member | null> {
    ...
    return (
      this.client.members.cache.get(buildMemberKey(serverId, userId)) ?? null
    );
  }
  ...
  async fetchSocialLinks(
    ...
    const data = await this.client.rest.router.socialLinks.memberSocialLinkRead(
      { serverId, userId, socialLinkType: type }
    );
    const existingMember = this.cache.get(buildMemberKey(serverId, userId));
    if (this.shouldCacheSocialLinks)
      existingMember?.socialLinks.set(data.socialLink.type, data.socialLink);
