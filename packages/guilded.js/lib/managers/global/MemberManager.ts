...
export class GlobalMemberManager extends CacheableStructManager<string, Member> {
    ...
    fetch(serverId: string, userId: string, force?: boolean): Promise<Member> {
        ...
        return this.client.rest.router.members.serverMemberRead({ serverId, userId }).then((data) => {
    ...
    fetchMany(serverId: string): Promise<Collection<string, PartialMember>> {
        return this.client.rest.router.members.serverMemberReadMany({ serverId }).then((data) => {
    ...
    kick(serverId: string, userId: string): Promise<Member | null> {
        return this.client.rest.router.members.serverMemberDelete({ serverId, userId }).then((_) => this.client.members.cache.get(buildMemberKey(serverId, userId)) ?? null);
    }
    ...
    getRoles(serverId: string, userId: string): Promise<number[]> {
        return this.client.rest.router.roleMembership.roleMembershipReadMany({ serverId, userId }).then((data) => data.roleIds);
    }
    ...
    resetNickname(serverId: string, userId: string): Promise<void> {
        return this.client.rest.router.members.memberNicknameDelete({ serverId, userId }).then(() => void 0);
    }
    ...
    giveXP(serverId: string, userId: string, amount: number): Promise<number> {
        return this.client.rest.router.serverXp.serverXpForUserCreate({ serverId, userId, requestBody: { amount } }).then((data) => data.total);
    }
    ...
    fetchSocialLinks(serverId: string, userId: string, type: SocialLinkPayload["type"]): Promise<SocialLinkPayload> {
        return this.client.rest.router.socialLinks.memberSocialLinkRead({ serverId, userId, socialLinkType: type }).then((data) => {
