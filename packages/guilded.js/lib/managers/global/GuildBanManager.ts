...
export class GlobalGuildBanManager extends CacheableStructManager<string, MemberBan> {
    ...
    fetch(serverId: string, userId: string, force?: boolean): Promise<MemberBan> {
        ...
        return this.client.rest.router.memberBans.serverMemberBanRead({ serverId, userId }).then((data) => {
    ...
    fetchMany(serverId: string): Promise<Collection<string, MemberBan>> {
        return this.client.rest.router.memberBans.serverMemberBanReadMany({ serverId }).then((data) => {
    ...
    ban(serverId: string, userId: string): Promise<MemberBan> {
        return this.client.rest.router.memberBans.serverMemberBanCreate({ serverId, userId }).then((data) => {
            const newMemberBan = new MemberBan(this.client, {
                serverId,
                ...data.serverMemberBan,
            });
    ...
    unban(serverId: string, userId: string, removeBanIfCached = false): Promise<MemberBan | null> {
        return this.client.rest.router.memberBans.serverMemberBanDelete({ serverId, userId }).then(() => {
