...
export class GlobalWebhookManager extends CacheableStructManager<string, Webhook> {
    ...
    create(serverId: string, options: OptionBody<WebhookService["webhookCreate"]>): Promise<Webhook> {
        return this.client.rest.router.webhook.webhookCreate({ serverId, requestBody: options }).then((data) => {
    ...
    fetchMany(serverId: string, channelId: string): Promise<Collection<string, Webhook>> {
        return this.client.rest.router.webhook.webhookReadMany({ serverId, channelId }).then((data) => {
    ...
    fetch(serverId: string, webhookId: string, force?: boolean): Promise<Webhook> {
        ...
        return this.client.rest.router.webhook.webhookRead({ serverId, webhookId }).then((data) => {
            const newWebhook = new Webhook(this.client, data.webhook);
            if (this.shouldCacheWebhook) this.cache.set(newWebhook.id, newWebhook);
            return newWebhook;
        });
    ...
    update(serverId: string, webhookId: string, options: OptionBody<WebhookService["webhookUpdate"]>): Promise<Webhook> {
        return this.client.rest.router.webhook.webhookUpdate({ serverId, webhookId, requestBody: options }).then((data) => {
