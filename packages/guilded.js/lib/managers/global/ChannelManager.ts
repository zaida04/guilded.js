...
export class GlobalChannelManager extends CacheableStructManager<string, Channel> {
	...
	async create(
		options: OptionBody<
			ChannelsService["channelCreate"]
		>,
	): Promise<Channel> {
		const data =
			await this.client.rest.router.channels.channelCreate(
				{
					requestBody: options,
				},
			);
		...
		return new (transformTypeToChannel(
			data
				.channel
				.type,
		))(
	...
	async fetch(
		...
		const data =
			await this.client.rest.router.channels.channelRead(
				{
					channelId,
				},
			);
		const fetchedChannel =
			new (transformTypeToChannel(
				data
					.channel
					.type,
			))(
	...
	async update(
		...
		const data =
			await this.client.rest.router.channels.channelUpdate(
				{
					channelId,
					requestBody: options,
				},
		...
		if (
			...
			return existingChannel._update(
				data.channel,
			);
 ...
 */
export const transformTypeToChannel = (
	str: ServerChannelPayload["type"],
): (typeof typeToChannel)[keyof typeof typeToChannel] =>
	typeToChannel[
		str as
			| "chat"
			| "docs"
