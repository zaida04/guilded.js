...
> {
  ...
  async create(
    ...
    return new (transformTypeToChannel(data.channel.type))(
      this.client,
      data.channel
    );
  }
  ...
  async delete(channelId: string): Promise<Channel | null> {
    await this.client.rest.router.channels.channelDelete({ channelId });
...
export const transformTypeToChannel = (
  ...
  typeToChannel[str as "chat" | "docs" | "forums" | "list"] ?? Channel;
