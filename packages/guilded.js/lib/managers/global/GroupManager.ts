import { GroupsService } from "@guildedjs/api";
...
export class GlobalGroupManager extends CacheableStructManager<string, Group> {
  ...
  create(
    serverId: string,
    options: OptionBody<GroupsService["groupCreate"]>
  ): Promise<Group> {
    return this.client.rest.router.groups
      .groupCreate({ serverId, requestBody: options })
      .then((data) => {
  ...
  update(
    serverId: string,
    groupId: string,
    options: OptionBody<GroupsService["groupUpdate"]>
  ): Promise<Group> {
    return this.client.rest.router.groups
      .groupUpdate({ serverId, groupId, requestBody: options })
      .then((data) => {
