...
export const Breadcrumb = () => {
	...
	const lib = query.lib as string | undefined;
	...
	const splittedPaths: Path[] = asPath.split("/").slice(3).map(x => ({ name: capitalize(x), path: `/docs/${lib}/${x}` }))
	...
	return <nav className="flex items-center justify-between flex-wrap text-white bg-black py-4 md:pl-40">
		<ol className="inline-flex items-center space-x-1 md:space-x-3">
			{combinedPaths.map((path, index, arr) => {
				const res = <li className="inline-flex items-center">
					<Link className={`inline-flex items-center text-sm font-medium ${path.isActive ? "text-guilded" : "text-white hover:text-guilded"}`} href={path.path}>
						{path.icon && <FontAwesomeIcon className="pr-2" icon={path.icon} />}
						{path.name}
					</Link>
				</li>

				if (index !== 0 && index !== arr.length) return <><BreadCrumbSep />{res}</>
				return res;
			})}
		</ol>
	</nav >
