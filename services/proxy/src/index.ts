...
import type { HTTPMethods } from "@guildedjs/api";
import { GuildedAPIError, RestManager } from "@guildedjs/api";
...
const server = createServer((req, res) => {
    ...
    if (!req.url || !req.method) {
        ...
        return res.end(
            JSON.stringify({
                error: { message: "You must supply a valid path and method." },
            }),
        );
    }
